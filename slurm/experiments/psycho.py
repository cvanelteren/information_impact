...
import numpy as np, itertools, copy, networkx as nx, pandas as pd
...
def setup(config : dict):
   # define type of model + settings
    ...
    n_magnetize = int(1e5)
    temps = np.linspace(.2, 4, 100);

    # fitting params
    maxfev = 100_000


    thetas = [.8, .5, .2] # match_temperatures


    # TODO : implement
    # graphs = [gen.generate(5)]
    fp = "/home/casper/projects/information_impact/Graphs/Graph_min1_1.csv"
    tmp = pd.read_csv(fp, header = 0, index_col = 0)
    g  = nx.convert_matrix.from_pandas_adjacency(tmp)
    

    graphs = [g for i in range(10)]
