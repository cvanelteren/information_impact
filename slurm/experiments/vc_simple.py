...
def setup(config : dict):
   # define type of model + settings
    ...
    combinations = itertools.product(thetas, graphs, vcs)
    for comb in combinations:
       ...
       instance_settings['agentStates'] = np.arange(len(vc))
       template = nx.complete_graph(len(vc))
       for i, j in template.edges():
            if vc.has_edge(i, j):
                weight = 1
            else:
                weight = -1
            template[i][j]['weight'] = weight

       instance_settings['rules'] = template
        # get params fit
       if phase_transition := phase_transitions.get((graph,vc)):
           opts, cov = phase_transition.get('fit')
            ...
            phase_transitions[(graph, vc)] = dict(fit = (opts, cov),
