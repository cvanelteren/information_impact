...
# TODO: make J weight to make model more general
...
class Model:
    def __init__(self, graph, agentStates, updateType= 'async', verbose = False):
        '''
        ...
        assert updateTypein self.modes
        # kinda uggly
        ...
        states     = np.zeros(nNodes, dtype = int)     # store state, H
#        edgeData   = zeros(nNodes, dtype = object)  # contains neighbors and weights [variable length]
        edgeData   = {}
        interaction= {}
        nudges     = np.zeros(nNodes, dtype = object)  # contains the nudge on a node [node x nudges]
        ...
        for node, nodeID in mapping.items():
         ...
         # set class data
        nodeIDs  = np.array(list(mapping.values()), dtype = int)
...
#            raise TypeError('type is not numpy array')
    ...
    def mode(self, value):
        '''
        When adjusting the mode, it checks if the updateTypeis possible
        '''
        if value not in self.modes:
            raise ValueError('updateTypeis not recognized, please use {}'.format(self.modes))
        else:
...
#        nodesToUpdate = random.choice(self.nodeIDs, size = (nSamples * step + 1, self.nNodes if self.updateType!= 'single' else 1))        # this seems faster  ^ when no while loop...
        if verbose:
            print(\
        ...
        # pbar = tqdm.tqdm(total = nSamples) # init progressbar
...
class SIR(Model):
    def __init__(self, graph, \
                beta = 1, gamma = 1, \
                agentStates = [-1, 1]):
        print('Init SIR model')
        '''
        agent states can be in one of three;
        susceptible, infected, recovered
        '''
        self.graph  = graph
        self.gamma  = gamma; self.beta = beta
        self.nudges = np.zeros(0, 1)
    def updateState(self):
        return 0
